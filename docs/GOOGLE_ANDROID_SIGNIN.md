...
The error **“add your debug SHA-1 in Google Cloud”** happens **before** your app can get a valid Google ID token. The backend cannot remove that; you must configure Google Cloud + the app as below.

## 1. Register SHA-1 in Google Cloud Console

1. Open [Google Cloud Console](https://console.cloud.google.com/) → the **same project** as your **Web client ID** (`app.google.client-id` in the backend).
2. **APIs & Services** → **Credentials**.
3. **Create credentials** → **OAuth client ID** → type **Android**.
4. **Package name:** `com.myledger.my_ledger_app` (must match `applicationId` in `android/app/build.gradle`).
5. **SHA-1 certificate fingerprint:** your **debug** keystore (for development):
   ...
   **Windows (PowerShell), from your Flutter/Android project root:**
   ...
   Or use your project’s script if you have one:
   ```powershell
...
That way the **ID token’s `aud` (audience)** matches what the backend verifies.

Do **not** use the Android client ID string as `requestIdToken` unless you also add that client ID to the backend:

```properties
# application.properties — optional, comma-separated
...
**Success:** same shape as OTP login — `data.accessToken`, `data.refreshToken`, `data.user`, etc.

## 4. Firebase (if you use it)

If the app uses Firebase Auth with Google:

- In Firebase Console → Project settings → your Android app → add the **same debug SHA-1**.
...
| Step | Action |
|------|--------|
| ☐ | SHA-1 from `gradlew signingReport` (debug) added to **Android** OAuth client |
| ☐ | Package name = `com.myledger.my_ledger_app` |
| ☐ | `requestIdToken` uses **Web** client ID (same as backend `app.google.client-id`) |
| ☐ | App calls `POST /myledger-api/auth/google/id-token` with `idToken` |
