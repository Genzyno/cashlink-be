# Google Sign-In – Step-by-Step Setup
...
2. **Browser** goes to: `http://localhost:4200/myledger-api/auth/google?redirect_uri=http%3A%2F%2Flocalhost%3A4200%2Fdashboard%2Foverview`
3. **Angular proxy** forwards that to: `http://localhost:8080/myledger-api/auth/google?redirect_uri=...`
4. **Backend** redirects the browser to **Google** sign-in.
5. **User** signs in with Google; Google redirects back to your **backend callback** (e.g. `http://localhost:8080/myledger-api/auth/google/callback`).
...
8. **Authorized redirect URIs** – add **exactly** the URL your backend uses for the Google callback, for example:
   - Local: `http://localhost:8080/myledger-api/auth/google/callback`  
   - Or whatever path your backend actually uses (e.g. `http://localhost:8080/myledger-api/auth/google/oauth2/callback`).
9. Save. Copy the **Client ID** and **Client Secret** – the backend will need these.
...
1. **Serve**  
   `GET /myledger-api/auth/google?redirect_uri=<url>`  
   (or `GET /auth/google` if your app is mounted under `/myledger-api`).
...
2. **On GET /auth/google:**
   ...
   - Redirect the browser to Google's authorization URL with:
     ...
     - `redirect_uri` = **your backend callback URL** (e.g. `http://localhost:8080/myledger-api/auth/google/callback`) – must match Google Console exactly
     - `response_type=code`
...
5. **Configuration** (e.g. `application.properties`):
   ...
   - `app.google.redirect-uri` = backend callback URL (must match Google Console exactly), e.g. `http://localhost:8080/myledger-api/auth/google/callback`
...
- **Redirect URL:**  
  ...
  So with `apiUrl = '/myledger-api/'` and origin `http://localhost:4200` you get:
  - `http://localhost:4200/myledger-api/auth/google?redirect_uri=http%3A%2F%2Flocalhost%3A4200%2Fdashboard%2Foverview`
- **Proxy:** `proxy.conf.js` forwards `/myledger-api` to `http://localhost:8080`, so the request hits the backend.
...
| Step | Check |
|------|--------|
| 1 | Google Cloud: OAuth 2.0 Web client created, **Authorized redirect URI** = backend callback URL (e.g. `http://localhost:8080/myledger-api/auth/google/callback`) |
...
- **404 on `/myledger-api/auth/google`**  
  Backend not running, or path not mounted under `/myledger-api`, or proxy target/port wrong in `proxy.conf.js`.
